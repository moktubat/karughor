                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                {items.map((item) => (
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            className="w-6 h-6 flex items-center justify-center bg-[#C85A3A] text-white rounded cursor-pointer hover:bg-[#A84830] transition-all flex-shrink-0"
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h3 className="text-base md:text-lg font-semibold text-[#0B0F0E] mb-1 line-clamp-1">
                                                {item.name}
                                            </h3>
                                            {item.originalPrice && (
                                                <p className="text-sm text-[#818B9C] line-through">৳{item.originalPrice}</p>
                                            )}
                                            <p className="text-lg font-semibold text-[#C85A3A] mt-1">৳{item.price}</p>
                                        </div>

                                        {/* Quantity */}
                                        <div className="flex items-center gap-3 border border-[#E4E9EE] rounded-lg px-3 py-2 flex-shrink-0">
                                            <button
                                        ...
                                        {/* Line total */}
                                        <p className="text-base font-bold text-[#0B0F0E] min-w-[80px] text-right flex-shrink-0">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white border border-[#E4E9EE] rounded-lg p-6 sticky top-6">
                            ...
                            <div className="flex flex-col gap-3 mb-4">
                                {items.map((item) => (
                                    <div key={item.id} className="flex justify-between items-center text-sm text-[#818B9C]">
                                        ...
                                        <span className="font-semibold text-[#0B0F0E] flex-shrink-0">
                                            ৳{item.price * item.quantity}
                                        </span>
                                    </div>
                                ))}
                            ...
                            <button
                                onClick={() =>
                                    router.push(`/checkout?delivery=${deliveryOption}`)
                                }
